...
async def create_inquiry(body: InquiryCreate):
    ...
    row = await db.fetch_one(
        '''INSERT INTO "Inquiry" (user_id, product_title, subject, name, email, quantity,
...
async def list_inquiries(status: str | None = Query(None)):
    if status:
        rows = await db.fetch_all(
            'SELECT * FROM "Inquiry" WHERE status = $1 ORDER BY id DESC', status
        )
    else:
        rows = await db.fetch_all('SELECT * FROM "Inquiry" ORDER BY id DESC')
    return [_serialize(r) for r in rows]


@router.get("/inquiries/{inquiry_id}")
async def get_inquiry(inquiry_id: int):
    row = await db.fetch_one('SELECT * FROM "Inquiry" WHERE id = $1', inquiry_id)
    if not row:
...
async def update_inquiry(inquiry_id: int, body: InquiryUpdate):
    inquiry = await db.fetch_one('SELECT * FROM "Inquiry" WHERE id = $1', inquiry_id)
    if not inquiry:
        raise HTTPException(status_code=404, detail="Inquiry not found")
        ...
        async with conn.transaction():
            ...
            if (
                ...
                placeholder = await conn.fetchrow(
                    'SELECT id FROM "Product" WHERE name = \'Custom Inquiry Order\' AND deleted_at IS NULL'
                )
                if not placeholder:
                    placeholder = await conn.fetchrow(
                        '''INSERT INTO "Product" (name, price, stock, active, created_at, updated_at)
                ...
                order = await conn.fetchrow(
                    '''INSERT INTO "Order" ("userId", total, currency, status, created_at, updated_at)
                       VALUES ($1,$2,'PHP','converted',$3,$3) RETURNING *''',
                    inquiry.get("user_id"), new_quoted, now,
                )
                await conn.execute(
                    '''INSERT INTO "OrderItem" ("orderId", "productId", quantity, unit_price, total_price, "createdAt")
                       VALUES ($1,$2,1,$3,$3,$4)''',
...
async def get_user_inquiries(user_id: int):
    rows = await db.fetch_all(
        'SELECT * FROM "Inquiry" WHERE user_id = $1 ORDER BY id DESC', user_id
    )
    return [_serialize(r) for r in rows]
...
async def convert_inquiry(inquiry_id: int):
    ...
    if inquiry.get("order_id"):
        order = await db.fetch_one('SELECT * FROM "Order" WHERE id = $1', inquiry["order_id"])
        return {"message": "Already converted", "orderId": inquiry["order_id"], "order": _serialize(dict(order)) if order else None}
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            await conn.execute(
                'UPDATE "Inquiry" SET order_id = $1, status = \'converted\', updated_at = $2 WHERE id = $3',
